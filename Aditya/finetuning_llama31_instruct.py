...
!pip install -q "transformers>=4.43" accelerate datasets bitsandbytes peft sentencepiece huggingface_hub sacrebleu rouge-score evaluate
...
# Llama 3.1 uses FP16 - we'll still use 4-bit quantization for P100 memory constraints
USE_4BIT_QUANTIZATION = True  # Set to False if you have enough GPU memory
...
def load_jsonl_data(file_path, tokenizer, max_samples=None, undersample_ratio=None):
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for idx, line in enumerate(f):
            ...
            # Use consistent Llama 3.1 Instruct format (matching tokenize_with_assistant_masking)
...
def tokenize_with_assistant_masking(examples, tokenizer):
    ...
    Uses Llama 3.1 Instruct format:
    <|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\n{system}<|eot_id|>
    <|start_header_id|>user<|end_header_id|>\n\n{user}<|eot_id|>
    <|start_header_id|>assistant<|end_header_id|>\n\n{assistant}<|eot_id|>
    """
    ...
    for messages in examples["messages"]:
        ...
        # Build the conversation in Llama 3.1's expected format
        # Start with begin_of_text token
        bos_tokens = tokenizer.encode("<|begin_of_text|>", add_special_tokens=False)
        full_ids.extend(bos_tokens)
        label_ids.extend([-100] * len(bos_tokens))
        ...
        for message in messages:
            ...
            eot_tokens = tokenizer.encode("<|eot_id|>", add_special_tokens=False)

            msg_tokens = header_tokens + content_tokens + eot_tokens
            full_ids.extend(msg_tokens)

            if role == 'assistant':
                # Only compute loss on assistant content (not header/eot)
                label_ids.extend([-100] * len(header_tokens))
                label_ids.extend(content_tokens)
                label_ids.extend(eot_tokens)  # Include eot in loss
            else:
...
def get_lora_target_modules(mode="attention_only"):
    """Get LoRA target modules based on strategy for Llama 3.1."""
    configs = {
        "attention_only": {
