!pip install -q "transformers>=4.35" accelerate datasets bitsandbytes peft sentencepiece huggingface_hub sacrebleu rouge-score wandb evaluate kaggle
...
MODEL_NAME = "/kaggle/input/mistral3/transformers/default/1"
DATA_PATH = "/kaggle/input/hrv-finetune/hrv_train.jsonl"
VAL_DATA_PATH = "/kaggle/input/hrv-finetune/hrv_test.jsonl"

# Hyperparameters optimized for P100 16GB
MAX_SEQ_LENGTH = 1536
LANGUAGE_BALANCE = True  # Enabled for multilingual balancing
...
class FocalLossCausalLMTrainer(Trainer):
    def __init__(self, *args, focal_gamma=2.0, yes_token_id=None, no_token_id=None, **kwargs):
        ...
        # Adjusted alpha for multilingual dataset distribution (~1:2 yes:no ratio)
        # Give more weight to "Yes" (violation) class since it's the minority
        self.alpha = torch.tensor([0.30, 0.70])  # [No weight, Yes weight]
...
def detect_language_improved(text):
    ...
    Supports: English, Russian (Cyrillic), Farsi/Persian (Arabic script),
    Spanish (Latin with accented characters).
    """
    if not text:
        return "unknown"
    ...
    arabic_chars = 0
    latin_chars = 0
    accented_latin_chars = 0  # Spanish-specific accented characters

    # Spanish-specific characters (common accented vowels, ñ, ¿, ¡)
    spanish_indicators = set('áéíóúüñ¿¡')
    ...
    for c in text:
        ...
        if 0x0400 <= o <= 0x04FF:
            ...
            latin_chars += 1
        if c.lower() in spanish_indicators:
            accented_latin_chars += 1

    total_chars = cyrillic_chars + arabic_chars + latin_chars
    if total_chars == 0:
        return "unknown"

    # Priority: Arabic script → Cyrillic → Spanish (accented Latin) → English
    if arabic_chars > 0 and arabic_chars / total_chars > 0.1:
        return "farsi"
    elif cyrillic_chars > 0 and cyrillic_chars / total_chars > 0.1:
        return "russian"
    elif latin_chars > 0:
        # Distinguish Spanish from English using accented chars
        if accented_latin_chars > 0 and accented_latin_chars / latin_chars > 0.01:
            return "spanish"
        else:
            return "english"
    else:
...
def convert_to_messages(entry):
    """Convert {instruction, input, output} format to ChatML messages format.

    The new multilingual dataset uses:
      - instruction: the task description (becomes system message)
      - input: the post to analyze (becomes user message)
      - output: the classification result (becomes assistant message)
...
def load_jsonl_data(file_path, tokenizer, max_samples=None, undersample_ratio=None):
    ...
    Supports both formats:
      ...
      - New multilingual: {instruction, input, output}
    """
    data = []
    stats = {
        "total": 0,
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for idx, line in enumerate(f):
            ...
            # Auto-detect format and convert to messages
            if 'messages' in entry:
                # Legacy format: already has messages
                messages = entry['messages']
            else:
                # New multilingual format: {instruction, input, output}
                messages = convert_to_messages(entry)

            # Extract user and assistant content for analysis
    ...
    if len(stats['user_languages']) >= 2:
        ...
        if ratio > 0.4:
            ...
            print("WARNING: Significant language imbalance detected!")

    # Class distribution analysis
    yes_data = [d for d in data if any(
        m['content'].strip().startswith('Yes') for m in d['messages'] if m['role'] == 'assistant'
    )]
    no_data = [d for d in data if any(
        m['content'].strip().startswith('No') for m in d['messages'] if m['role'] == 'assistant'
    )]

    print(f"\nClass distribution:")
    print(f"  Yes (Violation): {len(yes_data)}")
    print(f"  No (No Violation): {len(no_data)}")
    if len(yes_data) > 0:
        print(f"  Ratio (No:Yes): {len(no_data)/len(yes_data):.2f}:1")

    # Per-language class distribution
    print(f"\nPer-language class distribution:")
    for lang in sorted(stats['user_languages'].keys()):
        lang_yes = sum(1 for d in yes_data if d['language'] == lang)
        lang_no = sum(1 for d in no_data if d['language'] == lang)
        print(f"  {lang}: Yes={lang_yes}, No={lang_no}, Total={lang_yes+lang_no}")
    ...
    if undersample_ratio is not None:
        ...
        if target_no_samples < len(no_data):
            ...
            # Stratified undersampling: maintain language proportions in the majority class
            no_by_lang = {}
            for d in no_data:
                lang = d['language']
                no_by_lang.setdefault(lang, []).append(d)

            # Calculate per-language target counts proportionally
            total_no = len(no_data)
            sampled_no = []
            for lang, lang_samples in no_by_lang.items():
                lang_proportion = len(lang_samples) / total_no
                lang_target = max(1, int(target_no_samples * lang_proportion))
                lang_target = min(lang_target, len(lang_samples))
                sampled_no.extend(random.sample(lang_samples, lang_target))
        ...
        # Per-language distribution after undersampling
        print(f"\n  Per-language after undersampling:")
        for lang in sorted(stats['user_languages'].keys()):
            lang_count = sum(1 for d in data if d['language'] == lang)
        ...
        stats["class_distribution"] = {
            "yes": len(yes_data),
            "no": len(no_data),
            "ratio": len(no_data)/len(yes_data) if len(yes_data) > 0 else float('inf')
        }
...
def get_lora_target_modules(mode="attention_only"):
    ...
    configs = {
        "attention_only": {
            ...
            "description": "Conservative: Attention layers only (RECOMMENDED for multilingual data)"
        },
        "attention_mlp": {
...
def evaluate_hrv_classification(model, tokenizer, eval_dataset, device='cuda', max_new_tokens=50):
    ...
    Includes per-language breakdown for multilingual evaluation.
    """
    model.eval()
    predictions = []
    ground_truths = []
    ...
    print(f"\n{'='*60}")
    print("HRV Classification Evaluation Results (Overall):")
    print(f"{'='*60}")
    ...
    print(f"{'='*60}")
    print("Per-Language Evaluation Breakdown:")
    print(f"{'='*60}")
    ...
    for lang in unique_langs:
        ...
        lang_gt = [ground_truths[i] for i in lang_indices]
        lang_pred = [predictions[i] for i in lang_indices]
        lang_acc = accuracy_score(lang_gt, lang_pred)

        # Only compute if both classes exist for this language
        if len(set(lang_gt)) > 1:
            lang_p, lang_r, lang_f1, _ = precision_recall_fscore_support(
                lang_gt, lang_pred, average='binary', pos_label=1, zero_division=0
            )
        else:
            lang_p, lang_r, lang_f1 = 0.0, 0.0, 0.0
        ...
        per_lang_results[lang] = {
            ...
            'yes_count': sum(lang_gt),
            'no_count': len(lang_gt) - sum(lang_gt),
        }

        print(f"\n  {lang.upper()} ({len(lang_indices)} samples, Yes={sum(lang_gt)}, No={len(lang_gt)-sum(lang_gt)}):")
        print(f"    Accuracy:  {lang_acc:.4f}")
        print(f"    Precision: {lang_p:.4f}")
        print(f"    Recall:    {lang_r:.4f}")
...
training_args = TrainingArguments(
    ...
    run_name="openhermes-hrv-multilingual-finetuning",
    report_to="none",
...
# Evaluate on test data (all samples, up to 200 for speed)
...
try:
    eval_results = evaluate_hrv_classification(
        model=trainer.model,
        tokenizer=tokenizer,
        eval_dataset=val_data[:200],  # Evaluate on up to 200 test samples
